...
void test() {
    int K = 3; // number of filters
    int C = 10; // number of channels
    int H = 56; // height of the input image
    int W = 56; // width of the input image

    // Create filters and image with random values for testing purposes
    cube* filters = new cube[K];
    for (int i = 0; i < K; ++i) {
        filters[i] = cube(3, 3, C, fill::randu); // Using Armadillo's fill::randu for random initialization
    }
    cube image(H, W, C, fill::randu); // Using Armadillo's fill::randu for random initialization
