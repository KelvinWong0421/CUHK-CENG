...
parser = ArgParser(description="Assignment 3: super resolution convolutional neural network")
...
train_parser.add_argument("--num-epoch", type=int, default=200, help="number of epochs to train for")
train_parser.add_argument("--save-freq", type=int, default=100, help="save checkpoint every SAVE_FREQ epoches")
train_parser.add_argument("--lr", type=float, default=0.0000001, help="learning Rate. default=0.001")
...
test_set = SRDataset(collect_image(data_path["test"]))

train_data_loader = torch.utils.data.DataLoader(dataset=train_set, batch_size=opt.batch_size, shuffle=True,
                                                num_workers=num_workers)
test_data_loader = torch.utils.data.DataLoader(dataset=test_set, batch_size=1, shuffle=False,
                                               num_workers=0)
...
def test(epoch, data_loader):
    ...
    return psnr_metric.average()


# train num_epoch-start_epoch+1 epochs
for epoch in range(start_epoch, opt.num_epoch):
    loss=train(epoch, train_data_loader)
    psnr=test(epoch, test_data_loader)
    log["loss"].append(loss)
    log["psnr"].append(psnr)
    if (epoch + 1) % opt.save_freq == 0:
        # save a checkpoint
        save_checkpoint(epoch, model, optimizer, opt=opt, psnr=record["cur"], log=log)
        if export_plot:
            fig = make_subplots(
                rows=2, cols=1,
                subplot_titles=("Loss", "PNSR"))
            fig.add_trace(go.Scatter(x=list(range(len(log["loss"]))),y=log["loss"], name="Loss"),row=1, col=1)
            fig.add_trace(go.Scatter(x=list(range(len(log["psnr"]))),y=log["psnr"], name="PSNR"),row=2, col=1)
            fig.write_html('training_log.html', auto_open=False)

if record["model"] != None:
    # save best checkpoint
    print("Best PSNR={:.4f}".format(record["best"]))
    save_checkpoint(record["epoch"], record["model"], record["optim"], psnr=record["best"], best=True, opt=opt, log=log)
