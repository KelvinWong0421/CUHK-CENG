...
void handle_lui(unsigned int cur_inst) {
    ...
    int imm20 = MASK31_12(cur_inst) << 12;
...
void handle_lh(unsigned int cur_inst) {
    ...
    unsigned int rd = MASK11_7(cur_inst), rs1 = MASK19_15(cur_inst);
    int imm12 = MASK31_20(cur_inst);
   int base = sext(imm12, 12) + CURRENT_LATCHES.REGS[rs1];
   NEXT_LATCHES.REGS[rd] =(MEMORY[base +1] << 8) + \
                            (MEMORY[base]);
}
...
void handle_lw(unsigned int cur_inst) {
    ...
    int imm12 = MASK31_20(cur_inst);
    int base = sext(imm12, 12) + CURRENT_LATCHES.REGS[rs1];
NEXT_LATCHES.REGS[rd] = (MEMORY[base + 3] << 24) + \
                        (MEMORY[base + 2] << 16) + \
                        (MEMORY[base + 1] << 8) + \
                        MEMORY[base];
}
...
void handle_sb(unsigned int cur_inst) {
    ...
    int base = sext(imm12, 12) + CURRENT_LATCHES.REGS[rs1];
    MEMORY[base] = MASK7_0(CURRENT_LATCHES.REGS[rs2]);
}
...
void handle_sh(unsigned int cur_inst) {
    ...
    int base = sext(imm12, 12) + CURRENT_LATCHES.REGS[rs1];
    MEMORY[base] = MASK7_0(CURRENT_LATCHES.REGS[rs2]);
    MEMORY[base+1] = MASK15_8(CURRENT_LATCHES.REGS[rs2]);
}
...
void handle_sw(unsigned int cur_inst) {
    ...
    MEMORY[base] = MASK7_0(CURRENT_LATCHES.REGS[rs2]);
    MEMORY[base+1] = MASK15_8(CURRENT_LATCHES.REGS[rs2]);
    MEMORY[base+2] = MASK23_16(CURRENT_LATCHES.REGS[rs2]);
    MEMORY[base+3] = MASK31_24(CURRENT_LATCHES.REGS[rs2]);
}
        ...
        case (0x33) :                        //R-type
            switch(funct3){
                case 0:
                    ...
                    break;
                case 1: handle_sll(cur_inst);break;
                case 4: handle_xor(cur_inst);break;
                case 5:
                    if(MASK31_25(cur_inst) == 0){
                        handle_srl(cur_inst);
                    }else{
                    ...
                    break;
                case 6: handle_or(cur_inst);break;
                case 7: handle_add(cur_inst);break;
                default:error("unknown opcode 0x%08x is captured.\n", cur_inst);
            }break;
        case (0x3):             //load
            switch(funct3){
                case 0: handle_lb(cur_inst);break;
                case 1: handle_lh(cur_inst);break;
                case 2: handle_lw(cur_inst);break;
                default:error("unknown opcode 0x%08x is captured.\n", cur_inst);
            }break;
        case (0x23):            //S-type
            switch(funct3){
                case 0: handle_sb(cur_inst);break;
                case 1: handle_sh(cur_inst);break;
                case 2: handle_sw(cur_inst);break;
                default:error("unknown opcode 0x%08x is captured.\n", cur_inst);
            }break;
        case (0x63):                //B-type
            switch(funct3){
                case 0: handle_beq(cur_inst);break;
                case 1: handle_bne(cur_inst);break;
                case 4: handle_blt(cur_inst);break;
                case 5: handle_bge(cur_inst);break;
                default:error("unknown opcode 0x%08x is captured.\n", cur_inst);
            }break;
        case (0x6F): handle_jal(cur_inst);break;
        case (0x67): handle_jalr(cur_inst);break;
        case (0x37): handle_lui(cur_inst);break;
